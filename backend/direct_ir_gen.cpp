            ...
            for (int i = 0; i < max_iterations; i++) {
                ...
                if (condition["kind"] == "BinaryOperator") {
                    ...
                    if (condition.contains("inner") && condition["inner"].size() >= 2 &&
                        ...
                        std::string left_reg = left_var == target_var ? prev_iteration_value : vars[left_var];
                                ...
                                if (stmt["inner"][1]["kind"] == "BinaryOperator") {
                                    ...
                                    if (stmt["inner"][1].contains("inner") && 
                                        ...
                                        for (int j = 0; j < QBIT_WIDTH; j++) {
                                            ...
                                            prev_iteration_value = final_val;
...
for (int j = 0; j < QBIT_WIDTH; j++) {
    func.ops.push_back({
        ...
        // Copy the current value to the final diff register (diff3) for measurement 
...
void process_while_stmt(const nlohmann::json& while_json) {
    ...
    if (quantum_mode) {
        ...
        // Step 2: Find the variable that gets modified in the loop (target_var)
        std::string target_var;
        std::function<void(const nlohmann::json&)> find_modified_var =
            [&](const nlohmann::json& node) {
                if (node["kind"] == "BinaryOperator" && node.value("opcode", "") == "=") {
        ...
        if (!target_var.empty()) {
                ...
                std::function<void(const nlohmann::json&)> find_increment =
                    [&](const nlohmann::json& node) {
                        if (node["kind"] == "BinaryOperator" && 
                            node.value("opcode", "") == "=" && 
                            node.contains("inner") && node["inner"].size() >= 2) {
                            
                            std::string assigned_var;
                            if (node["inner"][0]["kind"] == "DeclRefExpr" && 
                                node["inner"][0].contains("referencedDecl")) {
                                assigned_var = node["inner"][0]["referencedDecl"].value("name", "");
                            }
                            ...
                            if (assigned_var == target_var && 
                                ...
                                if (node["inner"][1].contains("inner") && 
                                    ...
                                    if (node["inner"][1]["inner"][0]["kind"] == "DeclRefExpr" &&
                                        node["inner"][1]["inner"][0].contains("referencedDecl")) {
                                        left = node["inner"][1]["inner"][0]["referencedDecl"].value("name", "");
                                    }
                                    
                                    if (node["inner"][1]["inner"][1]["kind"] == "IntegerLiteral" &&
                                        node["inner"][1]["inner"][1].contains("value")) {
                                        right = node["inner"][1]["inner"][1]["value"].get<std::string>();
                                        increment_amount = right;
                                    } else if (node["inner"][1]["inner"][1]["kind"] == "DeclRefExpr" &&
                                               node["inner"][1]["inner"][1].contains("referencedDecl")) {
                                        right = node["inner"][1]["inner"][1]["referencedDecl"].value("name", "");
                                    }
            ...
            // Step 5: Now we'll unroll the loop for a fixed number of iterations
            // For the while loop structure: while (x < y) { x = x + 1; }
            ...
            for (int i = 0; i < max_iterations; i++) {
                ...
                // Step 5.1: Evaluate condition x < y and store in cond_reg[0]
                std::string diff = new_tmp("diff");
                emit_qubit_alloc(func, diff, QBIT_WIDTH);
                
                // Create subtractor circuit to compute right_var - left_var (y - x)
                // If result is positive, then x < y is true
                emit_quantum_subtractor(func, diff, vars[right_var], current_var, QBIT_WIDTH);
                ...
                // 3. AND the two conditions: diff is nonzero AND diff is non-negative
                // Use a Toffoli gate to AND both conditions
                func.ops.push_back({
                    QOpKind::Custom, cond_reg + "[0]",
                    nz + "[0]",
                    not_msb + "[0]",
                ...
                // Step 5.3: For the increment operation, we need to flip the appropriate bits
                // For x++ (incrementing by 1), this requires specific bit flips based on current value
                
                // Find the LSB and then conditionally flip bits depending on carries
                ...
                for (int j = 0; j < QBIT_WIDTH; j++) {
                    ...
                    if (j < QBIT_WIDTH - 1) {
                        // If current bit is 1 and carry in is 1, then carry out is 1
                        // Use a CCNOT (Toffoli) gate
                        func.ops.push_back({
                ...
                // Step 5.4: Now we need to conditionally apply the increment based on condition
                // Create a conditional register for the result
            ...
            // Step 6: Create the sum variable assignment (if present in the code)
            ...
            for (int j = 0; j < QBIT_WIDTH; j++) {
                func.ops.push_back({
                    QOpKind::Custom, "",
                    current_var + "[" + std::to_string(j) + "]",
                    vars[sum_var_name] + "[" + std::to_string(j) + "]",
                    0, "q.cx"
                });
            }
        }
