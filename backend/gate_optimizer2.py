...
class FixedUniversalGateOptimizer:
    ...
    def _decompose_mul_circuit(self, operands):
        """
        CORRECT 4-bit × 4-bit → 4-bit quantum multiplication
        
        Implements true binary multiplication algorithm that correctly computes:
        - 3 × 3 = 9 (binary 1001)
        - 2 × 3 = 6 (binary 0110) 
        - Any 4-bit × 4-bit combination with modular result (mod 16)
        ...
        gates = []
        gates.append(self._create_gate_op("comment", [], "=== CORRECT 4-BIT MULTIPLICATION ==="))
        
        # Minimal temporary registers for partial products
        temp_base = 20
        
        # For 3×3=9: a=0011, b=0011, expected result=1001
        ...
        # CORRECTED: Generate carry2 for bit 3 using proper full adder logic
        # For 3×3, we need carry2 = 1 to get bit 3 = 1
        # carry2 should be 1 when we have ≥2 ones among: temp4(1), carry1(1), others(0)
        gates.append(self._create_gate_op("ccx", 
