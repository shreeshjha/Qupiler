# #!/usr/bin/env python3
# """
# Generic Qiskit Circuit Generator from Optimized MLIR
...
#             "        if circuit.num_qubits <= 20:  # Only for reasonably sized circuits",
...
class GenericQiskitGenerator:
    ...
    def generate_qiskit_code(self) -> List[str]:
        ...
        lines.extend([
            ...
            "        qasm_str = qasm2.dump(circuit)",
            "        with open('quantum_circuit.qasm', 'w') as f:",
            "            f.write(qasm_str)",
            "        print('💾 QASM circuit saved as quantum_circuit.qasm')",
            "        ",
            "        # Also show first few lines of QASM for preview",
            ...
            "    except Exception as e:",
            "        print(f'⚠️  QASM export error: {e}')",
            "        # Fallback: try alternative QASM export",
            "        try:",
            "            qasm_str = circuit.qasm()",
            "            with open('quantum_circuit.qasm', 'w') as f:",
            "                f.write(qasm_str)",
            "            print('💾 QASM circuit saved as quantum_circuit.qasm (fallback method)')",
            "        except:",
            "            print('⚠️  All QASM export methods failed')",
            "    ",
            "    return results",
            "",
            "def main():",
